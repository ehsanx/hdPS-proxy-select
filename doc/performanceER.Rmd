---
title: "Summary of Performance Measures for Scenario ER"
author: "Ehsan, Yang"
...
load("results/scenarioER/bross.RData")
load("results/scenarioER/hybrid.RData")
load("results/scenarioER/lasso.RData")
load("results/scenarioER/enet.RData")
load("results/scenarioER/ga.RData")
load("results/scenarioER/xgboost.RData")
load("results/scenarioER/rf.RData")
load("results/scenarioER/forward.RData")
load("results/scenarioER/backward.RData")
load("results/scenarioER/kitchen.RData")

ind <- which(!is.na(RD_bross.ER[,1]))

est_bross    <- RD_bross.ER[ind,]
est_hybird   <- RD_hybrid_ER[ind,]
est_lasso    <- RD_lasso_ER[ind,]
est_enet     <- RD_enet_ER[ind,]
est_ga       <- RD_ga_ER[ind,]
est_xgboost  <- RD_xgboost_ER[ind,]
est_rf       <- RD_rf_ER[ind,]
...
ggsave(filename = "images/scenarioER/bias.png")
...
zip_plot + 
  ...
  dashed_line
ggsave(filename = "images/scenarioER/zip.png")
```
...
ggplot(melted_data, aes(x = model, y = value, fill = factor(variable, levels = c("empse", "modelse")))) +
  ...
  custom_theme
ggsave(filename = "images/scenarioER/secompare.png")
```
