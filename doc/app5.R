...
# setwd("C:/Users/Ehsan/Documents/GitHub/hdPS-proxy-select/doc")
#setwd(normalizePath("C:\Users\Ehsan\Documents\GitHub\hdPS-proxy-select\doc"))
...
setwd_to_script_dir <- function() {
  if (rstudioapi::isAvailable()) {
    ...
    if (nzchar(script_path)) {
      ...
      setwd(script_dir)
      message("Working directory set to script directory: ", script_dir)
    } else {
...
server <- function(input, output, session) {
  ...
  load_scenario_data <- function(scenario) {
    ...
    # Issue a warning if num_indices is greater than available bross_indices
    if (input$num_indices > length(bross_indices)) {
      showNotification(
        paste("Warning: Requested number of simulations (", input$num_indices, 
              ") exceeds available simulations (", length(bross_indices), "). Showing available data.", sep = ""),
        type = "warning",
        duration = 5
      )
      # Adjust num_indices to the available number of simulations
      num_indices <- length(bross_indices)
    } else {
      num_indices <- input$num_indices
    }
    
    bross_indices <- bross_indices[1:input$num_indices]
