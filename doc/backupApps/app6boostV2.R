...
server <- function(input, output, session) {
  ...
  load_scenario_data <- function(scenario, num_indices) {
    ...
    if (length(files) == 0) {
      showNotification("No data files found for the selected scenario.", type = "error")
      return(NULL)
    }
    ...
    if (length(est_list) == 0) {
      showNotification("No valid data available for the selected scenario.", type = "error")
      return(NULL)
    }
    ...
    if (!is.null(bross_indices)) {
      if (num_indices > length(bross_indices)) {
        showNotification(
          paste("Requested number of simulations (", num_indices, 
    ...
    if (is.null(est) || nrow(est) == 0) {
      showNotification("No data after filtering valid simulations.", type = "error")
      return(NULL)
    }
  ...
  selected_plot <- reactive({
    ...
    if (plot_type == "forest") {
      p <- p + labs(x = metric_names[internal_metric])
    }
  ...
  create_plot_download_handler <- function(filename, plot_function, extension) {
    downloadHandler(
      ...
      content = function(file) {
        plot <- plot_function()
        if (is.null(plot)) {
          showNotification("Cannot save plot: No plot available.", type = "error")
          return()
        }
        ggsave(file, plot = plot, device = extension, width = 8, height = 6)
      }
    )
  }
  ...
  output$download_latex <- downloadHandler(
    filename = function() { "performance_measures_table.tex" },
    content = function(file) {
      performance_measures <- performance_measures_reactive()
