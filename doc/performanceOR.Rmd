---
title: "Summary of Performance Measures for Scenario OR"
author: "Ehsan, Yang"
...
load("results/scenarioOR/bross.RData")
load("results/scenarioOR/hybrid.RData")
load("results/scenarioOR/lasso.RData")
load("results/scenarioOR/enet.RData")
load("results/scenarioOR/ga.RData")
load("results/scenarioOR/xgboost.RData")
load("results/scenarioOR/rf.RData")
load("results/scenarioOR/forward.RData")
load("results/scenarioOR/backward.RData")
load("results/scenarioOR/kitchen.RData")


ind <- which(!is.na(RD_bross.OR[,1]))

est_bross    <- RD_bross.OR[ind,]
est_hybird   <- RD_hybrid_OR[ind,]
est_lasso    <- RD_lasso_OR[ind,]
est_enet     <- RD_enet_OR[ind,]
est_ga       <- RD_ga_OR[ind,]
est_xgboost  <- RD_xgboost_OR[ind,]
est_rf       <- RD_rf_OR[ind,]
est_forward  <- RD_forward[ind,]
est_backward <- RD_backward[ind,]
est_kitchen  <- RD_kitchen[ind,]
...
est <- rbind(est_bross, est_hybird, est_lasso, est_enet, 
             ...
             est_forward, est_backward, est_kitchen)
est$model <- as.factor(est$model)
levels<- c("bross", "hybrid", "lasso", "enet", "ga", "xgboost", "rf", "forward", "backward", "kitchen")
...
ggsave(filename = "images/scenarioOR/bias.png")
...
zip_plot + 
  ...
  dashed_line
ggsave(filename = "images/scenarioOR/zip.png")
```
...
ggplot(melted_data, aes(x = model, y = value, fill = factor(variable, levels = c("empse", "modelse")))) +
  ...
  custom_theme
ggsave(filename = "images/scenarioOR/secompare.png")
```
