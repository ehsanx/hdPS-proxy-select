...
RD_enet_ER <- data.frame(numProxy = integer(a), RD = numeric(a), SE = numeric(a))
num.proxy_enet <- c()
# ---------------------------------------------------------------
...
for (i in 1:a) {
  ...
  RD_enet_ER[i,] <- sum.RD_enet
  rownames(RD_enet_ER)[i] <- paste0("data.", i, "_enet")
  ...
  save_dir <- "/scratch/st-mekarim-1/leiyang1/hdPS_ProxySelect/simResults_scenarioER/4_enet/"
...
save(RD_enet_ER, num.proxy_enet, avg.num.proxy_enet,
     file = "/scratch/st-mekarim-1/leiyang1/hdPS_ProxySelect/simResults_scenarioER/4_enet/enet.RData")
