...
```{r svm OR, warning=FALSE, cache=TRUE}
start_time <- Sys.time()
...
```{r svm OR-cont, warning=FALSE, cache=TRUE}
...
```{r svm RD, warning=FALSE, cache=TRUE}
fit_gau.svm <- glm(out.formula,
                     data = hdps.data,
                     weights = W.out.svm$weights,
                     family = gaussian(link= "identity"))

summary_rd.svm  <- summary(fit_gau.svm)$coef["exposure",
                                                 c("Estimate",
                                                   "Std. Error",
                                                   "Pr(>|t|)")]
summary_rd.svm[2]<- sqrt(sandwich::sandwich(fit_gau.svm)[2,2])

require(lmtest)
conf.int <- confint(fit_gau.svm, "exposure", level = 0.95,  method = "hc1")
summary_rd.svm  <- c(summary_rd.svm, conf.int)

round(summary_rd.svm, 2)
```
...
_(Long Running Time)_
```{r Boruta OR, warning=FALSE, cache=TRUE}
set.seed(42)
start_time <- Sys.time()
...
```{r Boruta RD, warning=FALSE, cache=TRUE}
fit_gau.boruta <- glm(out.formula,
                     data = hdps.data,
                     weights = W.out.boruta$weights,
                     family = gaussian(link= "identity"))

summary_rd.boruta  <- summary(fit_gau.boruta)$coef["exposure",
                                                 c("Estimate",
                                                   "Std. Error",
                                                   "Pr(>|t|)")]
summary_rd.boruta[2]<- sqrt(sandwich::sandwich(fit_gau.boruta)[2,2])

require(lmtest)
conf.int <- confint(fit_gau.boruta, "exposure", level = 0.95,  method = "hc1")
summary_rd.boruta  <- c(summary_rd.boruta, conf.int)

round(summary_rd.boruta, 2)
```
...
sum_ci.all <- rbind(sum_ci.bross, sum_ci.lasso, sum_ci.hybrid, sum_ci.elastic, 
                    sum_ci.svm, sum_ci.GA, 
                    sum_ci.xgboost, sum_ci.rf, sum_ci.boruta,
                    sum_ci.forward, sum_ci.backward, sum_ci.both)

OR.data <- data.frame(method = c("bross", "lasso", "hybrid", "elastic", 
                                 "SVM", "GA",
                                 "xgboost", "rf", "boruta", 
...
fs.all <- data.frame(numFeature = c(length(fs.bross), 
                     ...
                     numInCommon = c(NA,
                                     ...
                                     length(fs.backward[fs.backward %in% fs.bross]),
                                     length(fs.both[fs.both %in% fs.bross])))
rownames(fs.all) <-  c("bross", "lasso", "hybrid", "elastic", 
                       "SVM", "GA",
                       "xgboost", "rf", "boruta", 
                       "forward", "backward", "both")
...
fs.commonOneToOne <- data.frame(ini = rep(NA, 12))
...
for (i in 1:12) {
  ...
  fs.commonOneToOne[i] <- c(na, num_common[i:12])
}
...
```{r save RD}
# Save the entire workspace to a file named "workspaceNew.RData"
save.image(file = "data/workspaceNew.RData")
```
