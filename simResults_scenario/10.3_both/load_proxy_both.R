...
for (i in (b+1):a) {
  path_proxy <- paste0("/scratch/st-mekarim-1/leiyang1/hdPS_ProxySelect/simResults_scenario/10.3_both/proxy_both.data.", i, ".RData")
  ...
  if (file.exists(path_proxy)) {
    ...
    RD_both[i-b,] <- c(NA, NA, NA)
    rownames(RD_both)[i-b] <- paste0("data.", i, "_both")
    num.proxy_both <- c(num.proxy_both, NA)
    names(num.proxy_both)[i-b] <- paste0("data.", i, "_both")
  }
}

avg.num.proxy_both <- mean(num.proxy_both[complete.cases(num.proxy_both)])
RD_both_complete <- RD_both[complete.cases(RD_both),]
complete_ratio_both_both <- nrow(RD_both_complete)/nrow(RD_both)
incomplete_indices <- which(!complete.cases(RD_both))

# ---------------------------------------------------------------

save(RD_both, num.proxy_both, avg.num.proxy_both, complete_ratio_both_both, incomplete_indices,
