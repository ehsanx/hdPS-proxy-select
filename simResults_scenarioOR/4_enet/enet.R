...
RD_enet_OR <- data.frame(numProxy = integer(a), RD = numeric(a), SE = numeric(a))
num.proxy_enet <- c()
# ---------------------------------------------------------------
...
for (i in 1:a) {
  ...
  RD_enet_OR[i,] <- sum.RD_enet
  rownames(RD_enet_OR)[i] <- paste0("data.", i, "_enet")
  ...
  save_dir <- "/scratch/st-mekarim-1/leiyang1/hdPS_ProxySelect/simResults_scenarioOR/4_enet/"
...
save(RD_enet_OR, num.proxy_enet, avg.num.proxy_enet,
     file = "/scratch/st-mekarim-1/leiyang1/hdPS_ProxySelect/simResults_scenarioOR/4_enet/enet.RData")
