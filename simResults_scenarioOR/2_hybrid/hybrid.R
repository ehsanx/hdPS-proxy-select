...
RD_hybrid_OR <- data.frame(numProxy = integer(a), RD = numeric(a), SE = numeric(a))
num.proxy_hybrid <- c()
errors <- c()
# ---------------------------------------------------------------
...
for (i in 1:a) {
  ...
  if (class(hybrid.list) == "character") {
    ...
    RD_hybrid_OR[i,] <- sum.RD_hybrid
    rownames(RD_hybrid_OR)[i] <- paste0("data.", i, "_hybrid")
    ...
    save_dir <- "/scratch/st-mekarim-1/leiyang1/hdPS_ProxySelect/simResults_scenarioOR/2_hybrid/"
...
RD_hybrid_OR_no.error <- RD_hybrid_OR[complete.cases(RD_hybrid_OR),]
# ---------------------------------------------------------------

save(RD_hybrid_OR, num.proxy_hybrid, avg.num.proxy_hybrid, err_ratio, RD_hybrid_OR_no.error, 
     file = "/scratch/st-mekarim-1/leiyang1/hdPS_ProxySelect/simResults_scenarioOR/2_hybrid/hybrid.RData")
