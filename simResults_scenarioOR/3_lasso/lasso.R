...
RD_lasso_OR <- data.frame(numProxy = integer(a), RD = numeric(a), SE = numeric(a))
num.proxy_lasso <- c()
# ---------------------------------------------------------------
...
for (i in 1:a) {
  ...
  RD_lasso_OR[i,] <- sum.RD_lasso
  rownames(RD_lasso_OR)[i] <- paste0("data.", i, "_lasso")
  ...
  save_dir <- "/scratch/st-mekarim-1/leiyang1/hdPS_ProxySelect/simResults_scenarioOR/3_lasso/"
...
save(RD_lasso_OR, num.proxy_lasso, avg.num.proxy_lasso,
     file = "/scratch/st-mekarim-1/leiyang1/hdPS_ProxySelect/simResults_scenarioOR/3_lasso/lasso.RData")
