...
RD_ga_OR <- data.frame(numProxy = integer(a), RD = numeric(a), SE = numeric(a))
num.proxy_ga_OR <- c()
# ---------------------------------------------------------------
...
for (i in (b+1):a) {
  path_proxy <- paste0("/scratch/st-mekarim-1/leiyang1/hdPS_ProxySelect/simResults_scenarioOR/6_ga/proxy_ga.data.", i, ".RData")
  ...
  if (file.exists(path_proxy)) {
    ...
    RD_ga_OR[i-b,] <- sum.RD_ga
    rownames(RD_ga_OR)[i-b] <- paste0("data.", i, "_ga")
    num.proxy_ga_OR <- c(num.proxy_ga_OR, length(proxy_ga))
    names(num.proxy_ga_OR)[i-b] <- paste0("data.", i, "_ga")
    ...
    save_dir <- "/scratch/st-mekarim-1/leiyang1/hdPS_ProxySelect/simResults_scenarioOR/6_ga/"
    save_path <- paste0(save_dir, results_ga.i, ".RData")
    
    save(list = results_ga.i, file = save_path)
  } else {
    RD_ga_OR[i-b,] <- c(NA, NA, NA)
    rownames(RD_ga_OR)[i-b] <- paste0("data.", i, "_ga")
    num.proxy_ga_OR <- c(num.proxy_ga_OR, NA)
    names(num.proxy_ga_OR)[i-b] <- paste0("data.", i, "_ga")
  }
}

avg.num.proxy_ga_OR <- mean(num.proxy_ga_OR[complete.cases(num.proxy_ga_OR)])
RD_ga_OR_complete <- RD_ga_OR[complete.cases(RD_ga_OR),]
complete_ratio_ga_OR <- nrow(RD_ga_OR_complete)/nrow(RD_ga_OR)
incomplete_indices_ga_OR <- which(!complete.cases(RD_ga_OR))

# ---------------------------------------------------------------

save(RD_ga_OR, num.proxy_ga_OR, avg.num.proxy_ga_OR, complete_ratio_ga_OR, incomplete_indices_ga_OR,
     file = "/scratch/st-mekarim-1/leiyang1/hdPS_ProxySelect/simResults_scenarioOR/6_ga/ga.RData")
